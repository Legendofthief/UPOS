...
const GroupForm = ({ classes, ...props }) => {
    ...
    const onDelete = id => {
        if (window.confirm('Уверены ?'))
            props.deleteGroup(id,()=>addToast("Запись удалена", { appearance: 'info' }))
    }
    ...
    return(
      ...
      <Container className="d-flex flex-column">
         ...
         <tbody>
            {props.GroupList.map(record=>
             <tr key={record.id}>
                ...
                <td>{record.groupnm}</td>
                <td>{record?.faculty?.facultycode}</td>
                <td>{record?.faculty?.facultynm}</td>
                <td>
                             <div class="btn-toolbar">
                                <Button
                                  className="btn-light mr-1"
                                  onClick={()=>{setCurrentId(record.id);setModalGroupVisible(true) }}
                                >
        ...
        <ModalGroup 
        show={ModalGroupVisible} 
        onHide={() => setModalGroupVisible(false)}
        modalTitle="Редактор"
...
const mapActionToProps = {
    ...
    deleteGroup: actionsGroup.Delete
}

export default (
    connect(mapStateToProps, mapActionToProps)(GroupForm)
 ) ;
