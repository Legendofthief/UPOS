...
const DepartmentForm = ({ classes, ...props }) => {
    ...
    }, [ModalDepartmentVisible])//componentDidMount
    ...
    const onDelete = id => {
        if (window.confirm('Уверены ?'))
            props.deleteDepartment(id,()=>addToast("Запись удалена", { appearance: 'info' }))
    }
      ...
      <Container className="d-flex flex-column">
        <table className="table table-striped" >
         <thead>
          <tr>
            <th>Код кафедры</th>
            <th>Описание кафедры</th>
          </tr>
         </thead>
         <tbody>
            {props.DepartmentList.map(record=>
             <tr key={record.id}>
                <td>{record.departmentcode}</td>
                <td>{record.departmentnm}</td>
                <td>
                             <div class="btn-toolbar">
                                <Button
                                  className="btn-light mr-1"
                                  onClick={()=>{setCurrentId(record.id);setModalDepartmentVisible(true) }}
                                >
...
const mapActionToProps = {
    ...
    deleteDepartment: actionsDepartment.Delete
}

export default (
    connect(mapStateToProps, mapActionToProps)(DepartmentForm)
 ) ;
